...
<html xmlns:fb="http://www.facebook.com/2008/fbml">
  ...
  <body>
	...
	$from_name = cutname("$from_name");
      ...
      $temp = "photo";
      if ($type == "photo") {
      	$query = mysql_query("SELECT * FROM `facebook_photo` WHERE fb_id = '$fb_id'") or die(mysql_error());
    	$result = mysql_num_rows($query);
    	if (!empty($result)) {
    		$query1 = mysql_query("UPDATE facebook_photo Set `like`='$like' WHERE fb_id = '$fb_id'") or die(mysql_error());
      		} else {
      		$query2 = mysql_query("INSERT INTO `facebook_photo`(`fb_id`, `message`, `like`, `picture`, `original_link`, `created_time`, `fb_user`) VALUES ('$fb_id','$message','$like','$picture','$link','$created_time','$from_id')") or die(mysql_error());
      		}
      	}
